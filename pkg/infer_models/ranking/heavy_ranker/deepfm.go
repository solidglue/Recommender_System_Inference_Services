...
func (d *DeepFM) SetBaseModel(baseModel base_model.BaseModel) {
...
func (d *DeepFM) ModelInferSkywalking(model model_config.ModelConfig, requestId string, r *http.Request, inferSample feature.ExampleFeatures, retNum int) (map[string][]map[string]interface{}, error) {
...
func (d *DeepFM) ModelInferNoSkywalking(model model_config.ModelConfig, requestId string, inferSample feature.ExampleFeatures, retNum int) (map[string][]map[string]interface{}, error) {
	response := make(map[string][]map[string]interface{}, 0)
	cacheKeyPrefix := requestId + d.baseModel.GetServiceConfig().GetServiceId() + d.baseModel.GetModelName() + d.baseModel.GetTensorName()
	// get rsp from cache.
	rspDataBytes, err := d.baseModel.GetBigCacheRsp().Get(cacheKeyPrefix)
...
func (d *DeepFM) RecallSampleDirector(userId string, offlineFeature bool, onlineFeature bool, featureList []string) (feature.ExampleFeatures, error) {
	//panic("no not need to Implement interface methods")
	sample := feature.ExampleFeatures{}
	return sample, nil
}

func (d *DeepFM) RankingSampleDirector(userId string, offlineFeature bool, onlineFeature bool, itemIdList []string, featureList []string) (feature.ExampleFeatures, error) {
	//panic("no not need to Implement interface methods")
	sample := feature.ExampleFeatures{}
	return sample, nil
}
